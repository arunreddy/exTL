...
for d in itertools.permutations(domains, 2):
  ...
  orig_model = BinaryLogisticRegressionWithLBFGS(
    ...
    train_dir='output',
    log_dir='log',
    model_name='sentiment_{}_{}'.format(d[0], d[1]))

  orig_model.train()

  seed = 0
  np.random.seed(seed)
  a = np.random.randint(0, n_top, 200).tolist()
  b = np.random.randint(n_top, 2*n_top, 200).tolist()
  ...
  test_description = 'sentiment_{}_{}_{}'.format(md5.hexdigest(), d[0], d[1])
...
#print(tabulate(all_results, headers=['Source > Target', 'LR', 'SVM', 'SUBA', 'TCA', 'INF LR', 'INF SVM', 'INF SUBA', 'INF-TCA']))

print(tabulate(all_results, headers=['Source > Target', 'LR', 'SVM', 'SUBA', 'INF LR', 'INF SVM', 'INF SUBA']))