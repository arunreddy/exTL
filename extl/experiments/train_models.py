...
def evaluate(test_x, test_y, predictor, batchsize=100):
  ...
  for i in range(0, len(test_y), batchsize):
    x = chainer.Variable(xp.array(test_x[i:i + batchsize]))
...
def main():
  ...
  parser.add_argument('--iteration', '-i', type=int, default=10000,
                      help='Number of sweeps over the tdata to train')
  ...
  if not mytools.mkdir(args.out):
    ...
    return
  log_filename = results_dir + '/' + args.model_name + '_train_log.txt'
  ...
  for itr in range(args.iteration):
    ...
    if itr % log_freq == 0:
      ...
      print(
        "iteration: " + str(itr) + "  loss:" + str(mean_loss) + "  train_acc:" + str(train_acc) + "  test_acc:" + str(
          test_acc))
      mean_loss = 0
      denominator = 0

    if itr % (log_freq * 10) == 0:
      serializers.save_npz(results_dir + '/' + args.model_name, clf)
