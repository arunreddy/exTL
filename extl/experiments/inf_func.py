...
import os
from intl.experiments import mytools
from intl.models import lr
...
def main():
  ...
  dataset_name = 'txt_amazon'
  source = 'dvd'
  target = 'electronics'


  results_dir = os.path.join(os.environ['RESULTS_DIR'],dataset_name,'{}-{}'.format(source,target))


  model_path = os.path.join(results_dir,'model.npz')
  model_H_path = os.path.join(results_dir, 'model_H.npy')

  influence_scores_path = os.path.join(results_dir,'influence_scores.npz')



  # load classifier
  clf = lr.LogisticRegression()
  serializers.load_npz(model_path, clf)
  ...
  data = load_data_set(name=dataset_name, source=source, target=target)
  ...
  # calculate H
  if os.path.isfile(model_H_path):
    print('load H from ' + model_H_path)
    H = np.load(model_H_path)
  else:
    print('calculate H')
    H, acc = calc_H(train_x, train_y, clf, batchsize=batchsize, xp=xp)
    print('train accuracy: ' + str(acc))
    np.save(model_H_path, np.array(H))
  ...
  for i in tqdm(range(len(test_id))):
    ...
    # print('------------------\n\n')
    # print('Test ID:',test_id[i], test_y[test_id[i]])
    # print(dL0.shape, dL1.shape)

    # dL0 = dL0 / np.max(dL0)
    # dL1 = dL1 / np.max(dL1)

    influence_scores.append([dL0, dL1])

  np.savez_compressed(influence_scores_path, influence_scores=influence_scores)
    ...
    #
    #
    #   print('Class 0 +', dL0[arg0[0]], dL0[arg0[1]], dL0[arg0[2]])
    #   print('Class 0 -', dL0[arg0[-1]], dL0[arg0[-2]], dL0[arg0[-3]])
    #
    #   print('Class 1 +', dL1[arg1[0]], arg1[0], dL1[arg1[1]],arg1[1], dL1[arg1[2]], arg1[2])
    #   print('Class 1 -', dL1[arg1[-1]],  arg1[-1], dL1[arg1[-2]], arg1[-2],  dL1[arg1[-3]],  arg1[-3])
    #
    # else:
    #   imgs = [test_x[test_id[i]],  # test image
